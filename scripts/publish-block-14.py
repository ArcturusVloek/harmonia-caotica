...
def main() -> int:
    ...
    patch("sistemas/biblioteca.html", matriz_anchor, matriz_nova)
...
'''
    ...
    recuperacao_card = '<div class="system-category-card"><p class="system-category-card__eyebrow">Recuperação</p><h3>Curar, restaurar ou retornar</h3><p>Restaurar Vida ou Resistência, tratar Ferimentos, encerrar Condições, regenerar estruturas ou realizar Retorno.</p></div>'
    custos_card = recuperacao_card + '\n          <div class="system-category-card"><p class="system-category-card__eyebrow">Preço e combustível</p><h3>Pagar, armazenar ou sacrificar</h3><p>Definir custos, manutenção, Reservas, Recarga, Conversão, Recuperação e Sacrifícios.</p></div>'
    patch("sistemas/construcao-guiada.html", recuperacao_card, custos_card)
    cura_link = '<a href="cura-regeneracao-morte-e-retorno.html">Ler Cura e Retorno</a>'
    patch("sistemas/construcao-guiada.html", cura_link, cura_link + '<a href="custos-reservas-sacrificios-e-recuperacao.html">Ler Custos e Reservas</a>')
    memoria_row = '<tr><td>Apaga ou reescreve memória?</td><td>Aguarde o capítulo especializado.</td></tr>'
    custos_rows = '''<tr><td>Paga Vida, Resistência, Reserva ou capacidade?</td><td>Consulte Custos e registre pagador, quantidade e momento.</td></tr>
          ...
          ''' + memoria_row
    patch("sistemas/construcao-guiada.html", memoria_row, custos_rows)
    patch(
        "sistemas/construcao-guiada.html",
    ...
    nav13 = '<nav class="article-navigation" aria-label="Navegação da página"><a href="dano-ferimentos-condicoes-e-derrota.html">Bloco 12: Dano e Derrota</a><a href="index.html">Voltar aos Sistemas</a></nav>'
    nav14 = '<nav class="article-navigation" aria-label="Navegação da página"><a href="dano-ferimentos-condicoes-e-derrota.html">Bloco 12: Dano e Derrota</a><a href="custos-reservas-sacrificios-e-recuperacao.html">Bloco 14: Custos e Reservas</a><a href="index.html">Voltar aos Sistemas</a></nav>'
    patch("sistemas/cura-regeneracao-morte-e-retorno.html", nav13, nav14)

    workflow = ROOT / ".github/workflows/performance-pass.yml"
    workflow_text = workflow.read_text(encoding="utf-8")
    temporary_step = '''      - name: Integrar Bloco 14
        run: python3 scripts/publish-block-14.py

'''
    if temporary_step not in workflow_text:
        raise SystemExit("Etapa temporária não encontrada no fluxo de desempenho")
    workflow.write_text(workflow_text.replace(temporary_step, "", 1), encoding="utf-8")
